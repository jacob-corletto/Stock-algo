class Knapsack:
    ...
    def generate_knapsack_candidates(self, sublists):
        candidates = []

        def generate_knapsack(remaining_sublists, remaining_capacity, current_subset, max_item_count=1):
            ...
            for i in range(len(remaining_sublists)):
                ...
                if sublist[1] <= remaining_capacity:
                    ...
                    if max_item_count > 1:
                        generate_knapsack(remaining_sublists[:i] + remaining_sublists[i + 1:], remaining_capacity - sublist[1], new_subset, max_item_count - 1)
                    else:
                        generate_knapsack(remaining_sublists[1:], remaining_capacity - sublist[1], new_subset)

        generate_knapsack(sublists, self.capacity, [])
        return candidates
    ...
    def find_best(self):
        best_subset = []
        best_value = 0
        candidates = self.generate_knapsack_candidates(self.sublists)
